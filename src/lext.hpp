...
// Константы
const int BLOCK_SIZE = 1024;   // Размер блока в байтах
const int MAX_BLOCKS = 65536;   // Максимальное количество блоков
const int MAX_INODES = 16;    // Максимальное количество inode
const int INODE_SIZE = 1024;   // Размер inode в байтах
const int RECORDS_CNT = INODE_SIZE >> 2;
...
struct SuperBlock {
    ...
    uint32_t reserve[RECORDS_CNT-4];
};
...
struct Inode {
    ...
    uint32_t reserve[41];
    char filename[768];
    uint32_t direct_block_pointer[12];        // Прямые указатели на блоки данных
...
struct Inode_pointers {
    uint32_t block_pointer[256];        // Прямые указатели на блоки данных
};
...
private:
    ...
    uint32_t create(const std::string& name) {
        ...
        for (int i =0; i<12; i++) inode.direct_block_pointer[i] = -1;
...
public:
        ...
        for (uint32_t i = 0; i<inode_bitmap.size(); i++){
            if (inode_bitmap[i]){
                if (inodes[i].name_hash == hash_in){
                    ...
                    *seek_write = 0; // Seek(0) внутрений указатель в файле
    ...
    uint32_t next(){
     ...
     size_t write(uint32_t fd, const char* data, size_t size) {
        ...
        */
        ...
        for (int i = start_block_index; i < 12 && bytes_written < size; ++i) {
        ...
        for (int i = start_block_index - 12; i < RECORDS_CNT && bytes_written < size; ++i){
    ...
    size_t read(uint32_t fd, char* data, size_t size) {
        ...
        for (int i = start_block_index; i < 12 && inode.direct_block_pointer[i] != -1 && bytes_total_read < size; ++i) {
        ...
        if (inodes[fd].indirect1_pointer != -1){
            ...
            start_block_index = uint32_t(*seek / BLOCK_SIZE) ;
            in_block_offset = *seek % BLOCK_SIZE;
            for (int i = (start_block_index - 12); i < RECORDS_CNT && pointers->block_pointer[i] !=-1 && bytes_total_read < size; ++i){
...
//        }
    ...
    void rm(uint32_t fd) {
        ...
        *(uint64_t*)&inode.reserve[0] = 0;
        ...
        // Освобождение блоков
        for (int i = 0; i < 12 && inode.direct_block_pointer[i] != -1; ++i) {
        ...
        if (inode.indirect1_pointer != -1){
            ...
            for (int j=0; j<256 && j_pointers->block_pointer[j]!=-1 ;j++){
        ...
        if (inode.indirect2_pointer != -1){
            ...
            for (int j=0; j<256 && j_pointers->block_pointer[j]!=-1; j++){
                ...
                for (int i=0; i<256 && i_pointers->block_pointer[i]!=-1; i++){
